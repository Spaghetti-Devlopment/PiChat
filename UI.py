...
class serverWindow:
    ...
    def __init__(self, address, computerName, serverData):
        #Setting Up Window
        self.root = Tk()
        self.root.configure(bg="#121013")
        self.root.geometry("750x500")
        #Set Server Up
        serverData.serverUiWindow = self
        #Main UI
        self.makeServerWindow(address, computerName, self.normalX, self.normalY)

    def makeServerWindow(self, address, computerName, sizeX, sizeY):
        self.timesBiggerX = sizeX / self.normalX
        self.timesBiggerY = sizeY / self.normalY

        #Server Name
        serverName = Label(self.root, text="Server Names Are Not Available For Private Servers.", bg="#121013", fg="#9a9a9a", font=("Arial Bold", 15))
        serverName.place(x=0 * self.timesBiggerX, y=10 * self.timesBiggerY, width=750 * self.timesBiggerX, height=40 * self.timesBiggerY)
        #Title
        title= Label(self.root, text="Server Address: " + str(address) + " Computer Name: " + computerName, bg="#121013", fg="#9a9a9a", font=("Arial Bold", 15))
        title.place(x=0 * self.timesBiggerX, y=60 * self.timesBiggerY, width=750 * self.timesBiggerX, height=30 * self.timesBiggerY)

        self.serverLogsSetup()

        self.root.mainloop()

    def serverLogsSetup(self):
        #Make Canvas
        canvas = Canvas(self.root, bg="#121013")
        canvas.place(x=0 * self.timesBiggerX, y=110 * self.timesBiggerY, width=750 * self.timesBiggerX, height=300 * self.timesBiggerY)
        #Scroll
        scrollbar = Scrollbar(canvas)
        self.scrollbar = scrollbar
        #Message List
        ListBox = Listbox(canvas, yscrollcommand= self.scrollbar.set, bg="#121013", fg="#9a9a9a", font=("arial", 13), selectmode=BROWSE, height=100)
        self.messageList = ListBox
        self.messageList.insert(END, "Welcome To The Start Of Your Server Logs In PiChat")
        #Config Scroll Bar
        self.scrollbar.config(command = self.messageList.yview)
        self.scrollbar.pack(padx=(0, 0), pady=(0, 0), side = RIGHT, fill = Y)
        self.messageList.pack(fill = BOTH)

    def addToServerLogs(self, message):
       #Adding Messages
